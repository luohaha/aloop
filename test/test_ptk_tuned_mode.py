...
def test_ptk_tuned_mode_reduces_escape_timeouts(monkeypatch) -> None:
    ...
    # Redraw throttling should also be applied.
    assert handler.session.app.min_redraw_interval == 0.02
    assert handler.session.app.max_render_postpone_time == 0.02
...
def test_default_mode_keeps_prompt_toolkit_defaults(monkeypatch) -> None:
    ...
    # We don't touch redraw throttling in default mode.
    assert handler.session.app.min_redraw_interval is None
    assert handler.session.app.max_render_postpone_time == 0.01
